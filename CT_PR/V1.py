...
# Find j point in v1
def find_j_point_v1(waveform, qrs_peak):
    pass
...
class V1(func.PWave):
    ...
    def calculate_p_peaks(self, show=False, r=False):
        ...
        self.p_peak, self.p_amp, amps = self.find_p_peaks(polarity=None)
    ...
    def get_basic_t(self, show=False, j_point=None):
        ...
        self.t_peak, self.t_amp = self.t_obj.find_t_peak(polarity=0)
        self.ecg_result[self.anterior_leads]["T_Amplitude"] = {"V": round(self.t_amp, 4), "U": "mv"}
        self.t_obj.t_on_off()

        on_off_t = self.t_obj.t_onset_offset(show=False)
        ...
        if show:
            ...
            plt.step(np.arange(len(self.cleaned_signal)), self.cleaned_signal)
            plt.plot(np.arange(len(self.cleaned_signal)), self.cleaned_signal)
            # plt.stem(np.arange(len(self.cleaned_signal)), self.cleaned_signal)
