...
# Lead I calculation
class LeadII(func.PWave):
    def __init__(self, cleaned_signal, sampling_rate,amp_gain, anterior_leads=None):
        ...
        self.amp_gain =int(amp_gain)
        self.baseline = 0
        self.win_time = 20  # millisecond
    ...
    # Setting R peaks
    def set_r_peaks(self, r_pears, baseline=0, show=False, adjust_sig=True):
        """
        Param:
        ...
        if show:
            plt.title(f"QRS Peak Scatter Graph, {self.anterior_leads[0]}")
            plt.xlabel("Time")
    ...
    def get_qrs_area(self):
        qrs_area = []
        # print("Line 107: ", self.r_peaks["index"])
        for _, val in enumerate(self.r_peaks["index"]):
            li, lv = func.left_deep(self.cleaned_signal, val, lth=0)
            ri, rv = func.right_deep(self.cleaned_signal, val, lth=0)
            stp = val - li
            sop = val + ri
            area_range = sop - stp
            # print("Index 2: ", val, lv, rv)
            # print("QRS_Area: ", np.sum(self.cleaned_signal[stp:sop]) * self.cleaned_signal[val])
            # print("QRS_Area_Range: ", area_range, stp, sop)
    ...
    def rate_classification(self):
        ...
        self.rr_deviation = __heart.get_rr_deviation()
        
        
        self.ecg_result[self.anterior_leads]["R_Amplitude"] = {"V": round(r_amp/self.amp_gain, 4), "U": "mV","NR":data['R_Amplitude']}
        self.ecg_result[self.anterior_leads]["Q_Amplitude"] = {"V": round(q_amp/self.amp_gain, 4), "U": "mV","NR":data['Q_Amplitude']}
        self.ecg_result[self.anterior_leads]["S_Amplitude"] = {"V": round(s_amp/self.amp_gain, 4), "U": "mV","NR":data['S_Amplitude']}
        self.ecg_result[self.anterior_leads]["QRS_Duration"] = {"V": self._qrs_duration, "U": "ms","NR":data['QRS_Duration']}
        self.ecg_result[self.anterior_leads]["Q_Duration"] = {"V": __qrs.Q_duration, "U": "ms","NR":data['Q_Duration']}
        self.ecg_result[self.anterior_leads]["R_Duration"] = {"V": __qrs.R_duration, "U": "ms","NR":data['R_Duration']}
        self.ecg_result[self.anterior_leads]["S_Duration"] = {"V": __qrs.S_duration, "U": "ms","NR":data['S_Duration']}
        ...
        self.ecg_result[self.anterior_leads]["RR_Interval"] = {"V": round(self.rr_interval), "U": "ms","NR":data['RR_Interval']}
        self.ecg_result[self.anterior_leads]["RR_Deviation"] = {"V": round(self.rr_deviation), "U": "ms"}
        # self.ecg_result[self.anterior_leads]["Heart_Rate_Type"] = {"V": self.hr_type, "U": ""}
        self.ecg_result[self.anterior_leads]["Ventricular_rate"] = {"V": self.ventricular_rate, "U": "bpm","NR":data['Heart_Rate']}
        self.ecg_result[self.anterior_leads]["Heart_Rate"] = {"V": self.heart_rate, "U": "bpm","NR":data['Heart_Rate'] }
        ...
        #     self.qt_int = np.average(qt) * 0.010 * 1000
        #     # self.qtc_interval = round(np.mean(qt) + ((1000 - rr_interval) / 7))
        #     self.qtc_interval =  self.qt_int / np.sqrt(__heart.get_rr_interval()/1000)
        #     self.ecg_result[self.anterior_leads]["QT_Interval"] = {"V": round(self.qt_int), "U": "ms","NR":data['QT_Interval']}
        #     self.ecg_result[self.anterior_leads]["QTc_Interval"] = {"V": round(self.qtc_interval), "U": "ms","NR":data['QTc_Interval']}
        ...
        st = []
        s_offset = __qrs.get_s_offset()  # Assuming __qrs is not None
        t_offset = self.t_obj.get_t_offset()  # Assuming self.t_obj is not None

        if s_offset is not None and t_offset is not None:
            for s, t in zip(s_offset, t_offset):
                if t > s:
                    st.append(t - s) 
            # self.st_interval = round(np.mean(st))*0.010*1000
           
            if self.s_offset is not None:
                for ind, val in enumerate(self.s_offset):
                    if t_offset[ind] < val:
                        t_offset.pop(0)
                    else:
                        break

        st_int = np.average(st) * 0.010 * 1000

        if self.anterior_leads in self.ecg_result:
            self.ecg_result[self.anterior_leads]["ST_Interval"] = {"V": round(st_int), "U": "ms","NR":data['ST_Interval'] }

        return st_int, self.qt_int

        return self.st_int , self.qt_int
    ...
    def get_atrial_rate(self):
        ...
        pp_interval = round(np.average(np.diff([i[0] for i in self.p_peak])) * self.persamtim * 1000)
        ...
        self.ecg_result[self.anterior_leads]["pp_Interval"] = {"V": pp_interval, "U": "ms","NR":data['pp_Interval']}
    ...
    def get_pr_interval(self, q_onset, per_sample=0.0025):
        ...
        :return: pr duration
        """
        _P_duration = func.PWave(self.cleaned_signal, self.r_peaks["index"], margin=0, s_rate=100,conv_len=25, base_line=self.baseline, per_sample=0.0025)     
        # TODO: maybe need to improve the process of detection if required.
        self.q_onset = [i[0] for i in q_onset if len(i) > 0]
        self.ecg_result[self.anterior_leads]["P_Amplitude"] = {"V": round(np.average(self.avg)/self.amp_gain, 4), "U": "mV","NR":data['P_Amplitude']}
        ...
        #self.ecg_result[self.anterior_leads]["P_duration"] = {"V": self.P_duration  , "U": "ms","NR":data['P_Duration']}
        self.ecg_result[self.anterior_leads]["P_duration"] = {"V": self.P_duration  , "U": "ms","NR":data['P_Duration']}
      #uncomment 12-8-2023
        if np.average(self.avg) > 0.01:
            ...
            if len(self.pr_sample) > 2:
                ...
                self.ecg_result[self.anterior_leads]["PR_Interval"] = {"V": round(pr_int), "U": "ms","NR":data['PR_Interval']}
                return pr_int
            else:
                self.ecg_result[self.anterior_leads]["PR_Interval"] = {"V": 0, "U": "ms","NR":data['PR_Interval']}
                return 0
        else:
            # print("Cb")
            self.ecg_result[self.anterior_leads]["PR_Interval"] = {"V": 0, "U": "ms","NR":data['PR_Interval']}
    ...
    # T wave analysis
    def get_basic_t(self, show=False, j_point=None):
        ...
        self.ecg_result[self.anterior_leads]["T_Amplitude"] = {"V": round(self.t_amp/self.amp_gain, 4), "U": "mV","NR":data['T_Amplitude']}
        self.t_obj.t_on_off()

        on_off_t = self.t_obj.t_onset_offset()
...
with open('/home/datasets/test/single_lead_diagonosis/SNV.json', 'r') as json_file:
         # Load the JSON data
